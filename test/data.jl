using Compat, DataFrames, CategoricalArrays
## Dyestuff data from lme4
const ds = DataFrame(Any[ [1545.,1440.,1440.,1520.,1580.,1540.,1555.,1490.,1560.,1495.,
                           1595.,1550.,1605.,1510.,1560.,1445.,1440.,1595.,1465.,1545.,
                           1595.,1630.,1515.,1635.,1625.,1520.,1455.,1450.,1480.,1445.],
                     NominalArray(Compat.repeat('A' : 'F', inner = 5))], [:Yield, :Batch])

## Dyestuff2 data from lme4
const ds2 = DataFrame(Any[ [7.298,3.846,2.434,9.566,7.99,5.22,6.556,0.608,11.788,-0.892,
...
const slp = DataFrame(Any[
                      ...
                      Compat.repeat(0 : 9, outer = 18),
                      NominalArray(Compat.repeat(1 : 18, inner = 10))],
...
const psts = DataFrame(Any[ [62.8,62.6,60.1,62.3,62.7,63.1,60.0,61.4,57.5,56.9,61.1,58.9,
                       ...
                       NominalArray(bb), NominalArray(cc),
                       NominalArray(Compat.ASCIIString[string(b, c) for (b,c) in zip(bb,cc)])],
                      [:Strength, :Batch, :Cask, :Sample])

## Penicillin data from the lme4 package
const pen = DataFrame(Any[ [27,23,26,23,23,21,27,23,26,23,23,21,25,21,25,24,24,20,26,23,25,
